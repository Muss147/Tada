...
export default async function Page({
  ...
  const mission = await prisma.mission.findUnique({
    ...
    select: {
      ...
      // Récupérer les graphiques générés automatiquement avec leurs insights
      missionCharts: {
        where: {
          status: "published",
        },
    ...
    return (
      <div className="p-6 text-center">
        ...
        <p className="text-gray-500 mt-2">
          La mission demandée n’existe pas ou a été supprimée.
        </p>
      </div>
    );
  }
  ...
  if (mission.missionCharts && mission.missionCharts.length > 0) {
    // PRIORITÉ 1: Utiliser les graphiques de la base avec reformatage temps réel
    questionsData = mission.missionCharts
      .map((chart) => {
        const chartData = chart.chartData as any;

        if (!chartData?.question || !chartData?.type) {
          return null;
        }

        // Extraire les réponses temps réel pour cette question spécifique
        const questionResponses = extractResponsesForQuestion(
          surveyResponses,
          chartData.question
        );

        // Reformater les données temps réel
        const realTimeData = formatChartDataFromResponses(
          questionResponses,
          chartData.type
        );

        // Générer la config temps réel
        const realTimeConfig = generateChartConfigFromData(
          realTimeData,
          chartData.type
        );

        // Cas spécial pour les ratings : extraire primaryKeys et min/max
        ...
        return {
          ...
          min,
          max,
          // Ajouter les insights de la base
  ...
  // PRIORITÉ 2: Si pas de graphiques en base et mission pas live
  /*  if (questionsData.length === 0 && mission.status !== "live") {
    return (
      <div className="p-6 text-center">
        <h1 className="text-xl font-bold">Dashboard pas encore disponible</h1>
        <p className="text-gray-500 mt-2">
          Les graphiques seront générés automatiquement une fois la mission
          publiée.
        </p>
      </div>
    );
  ...
  // PRIORITÉ 3: Fallback - recalculer avec l'ancienne méthode
