...
export const duplicateMissionAction = authActionClient
  ...
  .action(async ({ parsedInput: { missionId }, ctx: { user } }) => {
    try {
      ...
      const baseName = `${originalMission.name} - duplicated`;

      const existingDuplications = await prisma.mission.findMany({
        where: {
          name: {
            startsWith: baseName,
          },
      ...
      const duplicationCount = existingDuplications.length;

      const newName =
        duplicationCount === 0
          ? baseName
          : `${baseName} (${duplicationCount + 1})`;
