export const createTooltipElement = () => {
  ...
  tooltipEl.className =
    "absolute pointer-events-none opacity-0 min-w-[120px] bg-white dark:bg-gray-800 rounded-lg shadow-md px-1 transition-all duration-150 ease-out";
...
export const generateTooltipContent = (
  ...
  bodyLines.forEach((body: string, i: number) => {
    ...
    let category: string, value: string;

    // Handle different chart types and data formats
    if (body.includes(":")) {
      const [cat, val] = body.split(":");
      category = cat || `Series ${i + 1}`;
      value = val || body;
    } else {
      category = `Series ${i + 1}`;
      value = body;
...
export const tooltipPlugin = {
  ...
  external: function (context: { tooltip: any; chart: any }) {
    ...
    const isCircularChart = ["pie", "doughnut", "polarArea", "radar"].includes(
      chartType
    );
    const isHorizontalBar =
      chartType === "bar" && context.chart.options.indexAxis === "y";
