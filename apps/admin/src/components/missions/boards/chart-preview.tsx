"use client";

import React, { useState, useEffect } from "react";
import { findIndex, isEqual } from "lodash";
import { TwitterPicker } from "react-color";
import { format } from "date-fns";
import { enGB } from "date-fns/locale";
import { LineChart } from "./widgets/charts/line-chart";
import BarChart from "./widgets/charts/bar-chart";
import PieChart from "./widgets/charts/pie-chart";
import { DoughnutChart } from "./widgets/charts/doughnut-chart";
import { RadarChart } from "./widgets/charts/radar-chart";
import { PolarChart } from "./widgets/charts/polar-chart";
import { KpiMode } from "./widgets/charts/kpi-mode-chart";
import { GaugeChart } from "./widgets/charts/gauge-chart";
import { chartColors } from "@/constants/chart";
import { Button } from "@tada/ui/components/button";
import {
  CircleX,
  CircleXIcon,
  ListFilter,
  Loader2,
  PlusIcon,
  RefreshCw,
  Gauge,
  ChartLine,
  ChartPie,
  Table2Icon,
  ChartBarIcon,
  PieChartIcon,
  LineChartIcon,
  DonutIcon,
  RadarIcon,
  HashIcon,
  CalculatorIcon,
} from "lucide-react";
import { Input } from "@tada/ui/components/input";
import { Chart } from "@/context/board-builder-context";
import { Badge } from "@tada/ui/components/badge";
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
  TooltipTrigger,
} from "@tada/ui/components/tooltip";
import { Skeleton } from "@/components/skeleton";

interface ChartDatasetConfig {
  Dataset?: {
    conditions?: Array<{
      exposed: boolean;
      id: string;
      type: string;
      value: string | number;
    }>;
  };
}

interface GaugeRange {
  min: number;
  max: number;
  label: string;
  color: string;
}

interface Condition {
  id: string;
  type: string;
  value: string | number;
}

interface ChartPreviewProps {
  chart: Partial<Chart>;
  chartLoading: boolean;
  onChange: (data: Partial<Chart>) => void;
  onRefreshData: (conditions?: Condition[] | boolean) => void;
  onRefreshPreview: () => void;
  changeCache: (value: boolean) => void;
  useCache: boolean;
  onAddFilter: (condition: Condition) => void;
  onClearFilter: (condition: Record<string, string>) => void;
}

export function ChartPreview({
  chart,
  onChange,
  onRefreshData,
  onRefreshPreview,
  chartLoading,
  changeCache,
  useCache,
}: ChartPreviewProps) {
  const [redraw, setRedraw] = useState<boolean>(false);
  const [conditions, setConditions] = useState<Condition[]>([]);
  const [ranges, setRanges] = useState<GaugeRange[]>([]);
  const [rangeErrors, setRangeErrors] = useState<string | null>(null);

  useEffect(() => {
    _onRefreshPreview();
  }, [chart.type]);

  useEffect(() => {
    setRedraw(true);
  }, [chart.dataLabels]);

  useEffect(() => {
    setRanges(
      chart.ranges
        ? (chart.ranges as unknown as { value: GaugeRange[] }).value
        : [
            {
              min: 0,
              max: 100,
              label: "Total",
              color: Object.values(chartColors)[0]?.hex || "",
            },
          ]
    );
  }, [chart.ranges]);

  const _checkIfFilters = () => {
    let filterCount = 0;
    /*     chart.ChartDatasetConfigs.forEach((d) => {
      if (Array.isArray(d.Dataset?.conditions)) {
        filterCount += d.Dataset.conditions.filter((c) => c.exposed).length;
      }
    }); */

    return filterCount > 0;
  };

  const _onAddFilter = (condition: Condition) => {
    let found = false;
    const newConditions = conditions.map((c) => {
      let newCondition = c;
      if (c.id === condition.id) {
        newCondition = condition;
        found = true;
      }
      return newCondition;
    });
    if (!found) newConditions.push(condition);
    setConditions(newConditions);

    onRefreshData(newConditions);
  };

  const _onClearFilter = (condition: Condition) => {
    const newConditions = [...conditions];
    const clearIndex = findIndex(conditions, { id: condition.id });
    if (clearIndex > -1) newConditions.splice(clearIndex, 1);

    setConditions(newConditions);

    onRefreshData(newConditions);
  };

  const _onChangeChartType = (data: Partial<Chart>) => {
    const newType = data;
    if (
      data.type === "polar" ||
      data.type === "pie" ||
      data.type === "doughnut" ||
      data.type === "radar" ||
      data.type === "table"
    ) {
      newType.subType = "timeseries";
      newType.mode = "chart";
    }

    if (data.type === "avg" && chart.type !== "avg") {
      newType.subType = "timeseries";
    } else if (data.type === "avg" && chart.type === "avg") {
      newType.subType = "timeseries";
      newType.mode = "chart";
      newType.type = "line";
    }

    return onChange(newType);
  };

  const _toggleAccumulation = () => {
    if (!chart) return;
    if (chart.subType && chart.subType?.indexOf("AddTimeseries") > -1) {
      return onChange({ subType: "timeseries" });
    }

    const updateData = { subType: "AddTimeseries", type: "" };
    if (chart.type === "avg") updateData.type = "line";

    return onChange(updateData);
  };

  const _onChangeMode = () => {
    setRedraw(true);

    return onChange({ mode: chart.mode === "chart" ? "kpichart" : "chart" });
  };

  const _onChangeGrowth = () => {
    setRedraw(true);

    return onChange({ showGrowth: !chart.showGrowth });
  };

  const _onChangeInvertGrowth = () => {
    setRedraw(true);

    return onChange({ invertGrowth: !chart.invertGrowth });
  };

  const _redrawComplete = () => {
    setRedraw(false);
  };

  const _onRefreshPreview = () => {
    setRedraw(true);
    onRefreshPreview();
  };

  const _onRefreshData = () => {
    setRedraw(true);
    onRefreshData(useCache);
  };

  const _onChangeRange = (
    value: number | string,
    index: number,
    key: keyof GaugeRange
  ) => {
    const newRanges = ranges.map((range, i) => {
      if (i === index) {
        const newRange = { ...range } as any;
        newRange[key] = value;
        return newRange;
      }
      return range;
    });
    setRanges(newRanges);
  };

  const _onAddRange = () => {
    const previousRange = ranges[ranges.length - 1] || {
      min: 0,
      max: 100,
      label: "Total",
      color: Object.values(chartColors)[0]?.hex,
    };

    // Get next available color
    const colorIndex = ranges.length % Object.values(chartColors).length;

    setRanges([
      ...(ranges || []),
      {
        min: previousRange.max,
        max: previousRange.max + 20,
        label: `${previousRange.max}-${previousRange.max + 20}`,
        color: Object.values(chartColors)[colorIndex]?.hex || "",
      },
    ]);
  };

  const _onRemoveRange = (index: number) => {
    if (ranges.length === 1) return;
    const newRanges = [...ranges];
    newRanges.splice(index, 1);
    setRanges(newRanges);
  };

  const _onChangeColor = (color: { hex: string }, index: number) => {
    const newRanges = ranges.map((range, i) => {
      if (i === index) {
        return { ...range, color: color.hex };
      }
      return range;
    });
    setRanges(newRanges);
  };

  const _onSaveRanges = () => {
    setRangeErrors(null);
    // do a sanity check on the ranges
    // ensure that min is always less than max
    if (ranges.some((range) => range.min > range.max)) {
      setRangeErrors("Min values must be less than max values");
      return;
    }

    // ensure that the ranges are not overlapping
    if (
      ranges.some((range, index) =>
        ranges.some(
          (r, i) => i !== index && range.min < r.max && range.max > r.min
        )
      )
    ) {
      setRangeErrors("Ranges cannot overlap");
      return;
    }

    // ensure that the ranges are not empty
    if (ranges.some((range) => range.min === null || range.max === null)) {
      setRangeErrors("Ranges cannot be empty");
      return;
    }

    // ensure the labels are not empty
    if (ranges.some((range) => range.label === null)) {
      setRangeErrors("Labels cannot be empty");
      return;
    }

    onChange({ ranges: { value: ranges } as any });
  };

  return (
    <div className={"bg-white dark:bg-slate-600 rounded-lg mx-auto p-4 w-full"}>
      {chart && chart.chartData && (
        <>
          <div className={"w-full"}>
            <div className="flex flex-between items-center ">
              <div className="flex items-center gap-1">
                <Button onClick={_onRefreshData} size="sm">
                  {chartLoading ? (
                    <Loader2 className="w-4 h-4" />
                  ) : (
                    <div className="flex items-center gap-1">
                      <span>Refresh chart</span>
                      <RefreshCw className="w-4 h-4" />
                    </div>
                  )}
                </Button>

                <div className="h-4" />
              </div>

              {_checkIfFilters() && (
                <>
                  <Button variant="ghost" size="sm">
                    <ListFilter />
                  </Button>

                  {/*   <ChartFilters
                    chart={chart}
                    onAddFilter={_onAddFilter}
                    onClearFilter={_onClearFilter}
                    conditions={conditions}
                  /> */}
                </>
              )}
            </div>

            {chart.ChartDatasetConfigs && conditions.length > 0 && (
              <>
                <div className="h-8" />
                <div className="flex items-center gap-1">
                  {chart.ChartDatasetConfigs &&
                    conditions.map((c) => (
                      <Badge key={c.id} className="flex items-center gap-1">
                        <span>
                          {c.type !== "date" && `${c.value}`}
                          {c.type === "date" &&
                            format(new Date(c.value), "Pp", { locale: enGB })}
                        </span>
                        <Button
                          onClick={() => _onClearFilter(c)}
                          className="text-default-500 flex items-center"
                        >
                          <CircleX size={14} />
                        </Button>
                      </Badge>
                    ))}
                </div>
              </>
            )}
            <div className="h-8" />
            <div className="flex ">
              <div className="w-full h-px bg-border" />
            </div>

            <div className="h-8" />
            <div className="h-[300px] w-full">
              {chart.type === "line" && (
                <LineChart
                  editMode
                  chart={chart as any}
                  redraw={redraw}
                  redrawComplete={_redrawComplete}
                />
              )}
              {chart.type === "bar" && (
                <BarChart
                  editMode
                  chart={chart as any}
                  redraw={redraw}
                  redrawComplete={_redrawComplete}
                />
              )}
              {chart.type === "pie" && (
                <PieChart chart={chart as any} /* height={300} editMode */ />
              )}
              {chart.type === "doughnut" && (
                <DoughnutChart
                  chart={chart as any} /*  height={300} editMode */
                />
              )}
              {chart.type === "radar" && (
                <RadarChart chart={chart as any} /* height={300} editMode */ />
              )}
              {chart.type === "polar" && (
                <PolarChart chart={chart as any} /* height={300} editMode */ />
              )}
              {chart.type === "table" && (
                <div className="h-full">
                  {/*  <TableContainer
                    tabularData={chart.chartData}
                    datasets={chart.ChartDatasetConfigs}
                    height={400}
                    editMode
                  /> */}
                </div>
              )}

              {(chart.type === "kpi" || chart.type === "avg") && (
                <KpiMode chart={chart as any} /* editMode */ />
              )}

              {chart.type === "gauge" && (
                <GaugeChart
                  chart={chart as any}
                  redraw={redraw}
                  redrawComplete={_redrawComplete}
                />
              )}
            </div>
          </div>
          <div className="h-8" />
          <div className="border-solid border-1 border-content3 px-3 py-2 rounded-2xl chart-preview-types">
            <div className="flex items-center flex-wrap gap-4 justify-around">
              <TooltipProvider>
                <div className=" flex gap-1">
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
                        variant={chart.type !== "avg" ? "outline" : "secondary"}
                        color={chart.type === "avg" ? "secondary" : "default"}
                        onClick={() => _onChangeChartType({ type: "avg" })}
                      >
                        <CalculatorIcon size={24} />
                      </Button>
                    </TooltipTrigger>
                    <TooltipContent>
                      Get the average value of all the points on the chart
                    </TooltipContent>
                  </Tooltip>

                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
                        variant={
                          chart.subType &&
                          chart.subType?.indexOf("AddTimeseries") === -1
                            ? "outline"
                            : "secondary"
                        }
                        color={
                          chart.subType &&
                          chart.subType?.indexOf("AddTimeseries") > -1
                            ? "secondary"
                            : "default"
                        }
                        onClick={_toggleAccumulation}
                        disabled={
                          chart.type !== "line" &&
                          chart.type !== "bar" &&
                          chart.type !== "avg" &&
                          chart.type !== "kpi" &&
                          chart.type !== "gauge"
                        }
                      >
                        <ChartLine size={20} />
                      </Button>
                    </TooltipTrigger>
                    <TooltipContent>
                      {chart.subType &&
                      chart.subType?.indexOf("AddTimeseries") > -1
                        ? "Turn accumulation off"
                        : "Accumulate datasets"}
                    </TooltipContent>
                  </Tooltip>
                </div>

                <div className=" flex gap-1">
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
                        variant={
                          chart.type !== "table" ? "outline" : "secondary"
                        }
                        onClick={() => _onChangeChartType({ type: "table" })}
                        color={chart.type === "table" ? "primary" : "default"}
                      >
                        <Table2Icon size={20} />
                      </Button>
                    </TooltipTrigger>
                    <TooltipContent>
                      Display data in a table view
                    </TooltipContent>
                  </Tooltip>

                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
                        variant={
                          chart.type !== "line" ? "outline" : "secondary"
                        }
                        onClick={() => _onChangeChartType({ type: "line" })}
                        color={chart.type === "line" ? "primary" : "default"}
                      >
                        <LineChartIcon size={24} />
                      </Button>
                    </TooltipTrigger>
                    <TooltipContent>Display as line chart</TooltipContent>
                  </Tooltip>

                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
                        variant={chart.type !== "bar" ? "outline" : "secondary"}
                        onClick={() => _onChangeChartType({ type: "bar" })}
                        color={chart.type === "bar" ? "primary" : "default"}
                      >
                        <ChartBarIcon size={24} />
                      </Button>
                    </TooltipTrigger>
                    <TooltipContent>Display as bar chart</TooltipContent>
                  </Tooltip>

                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
                        variant={chart.type !== "kpi" ? "outline" : "secondary"}
                        onClick={() => _onChangeChartType({ type: "kpi" })}
                        color={chart.type === "kpi" ? "primary" : "default"}
                      >
                        <HashIcon size={24} />
                      </Button>
                    </TooltipTrigger>
                    <TooltipContent>
                      Display as a single value (KPI)
                    </TooltipContent>
                  </Tooltip>

                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
                        variant={
                          chart.type !== "gauge" ? "outline" : "secondary"
                        }
                        onClick={() => _onChangeChartType({ type: "gauge" })}
                        color={chart.type === "gauge" ? "primary" : "default"}
                      >
                        <Gauge size={24} />
                      </Button>
                    </TooltipTrigger>
                    <TooltipContent>Display as a gauge chart</TooltipContent>
                  </Tooltip>
                </div>

                <div className=" flex gap-1">
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
                        variant={chart.type !== "pie" ? "outline" : "secondary"}
                        onClick={() => _onChangeChartType({ type: "pie" })}
                        color={chart.type === "pie" ? "primary" : "default"}
                      >
                        <PieChartIcon size={24} />
                      </Button>
                    </TooltipTrigger>
                    <TooltipContent>Display as pie chart</TooltipContent>
                  </Tooltip>

                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
                        variant={
                          chart.type !== "doughnut" ? "outline" : "secondary"
                        }
                        onClick={() => _onChangeChartType({ type: "doughnut" })}
                        color={
                          chart.type === "doughnut" ? "primary" : "default"
                        }
                      >
                        <DonutIcon size={24} />
                      </Button>
                    </TooltipTrigger>
                    <TooltipContent>Display as doughnut chart</TooltipContent>
                  </Tooltip>

                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
                        variant={
                          chart.type !== "radar" ? "outline" : "secondary"
                        }
                        onClick={() => _onChangeChartType({ type: "radar" })}
                        color={chart.type === "radar" ? "primary" : "default"}
                      >
                        <RadarIcon size={24} />
                      </Button>
                    </TooltipTrigger>
                    <TooltipContent>Display as radar chart</TooltipContent>
                  </Tooltip>

                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
                        variant={
                          chart.type !== "polar" ? "outline" : "secondary"
                        }
                        onClick={() => _onChangeChartType({ type: "polar" })}
                        color={chart.type === "polar" ? "primary" : "default"}
                      >
                        <ChartPie size={24} />
                      </Button>
                    </TooltipTrigger>
                    <TooltipContent>Display as polar chart</TooltipContent>
                  </Tooltip>
                </div>
              </TooltipProvider>
            </div>
          </div>
        </>
      )}

      <div>
        {chart && chart.type && !chart.chartData && (
          <>
            {chartLoading && (
              <>
                <div className="flex">
                  {/* <CircularProgress size="lg" aria-label="Loading chart data" /> */}
                </div>

                <div className="flex">
                  <span className="text-default-500 font-bold">
                    Loading chart data...
                  </span>
                </div>
                <div className="my-8" />
              </>
            )}
            {!chartLoading && (
              <>
                <div className={"container mx-auto"}>
                  <span className="text-default-500 font-bold">
                    Configure the dataset to get started
                  </span>

                  <div className="h-4" />
                  <Skeleton className="rounded-lg">
                    <div className="h-5 rounded-lg bg-default-300"></div>
                  </Skeleton>
                </div>
                <div className="h-2" />
              </>
            )}
          </>
        )}
      </div>

      {chart &&
        chart.type &&
        chart.ChartDatasetConfigs &&
        chart.ChartDatasetConfigs.length > 0 &&
        chart.type !== "gauge" && (
          <div style={styles.topBuffer} className="chart-preview-growth">
            <div className="flex flex-row items-center gap-4">
              <label className="flex items-center gap-2">
                <input
                  type="checkbox"
                  checked={chart.mode === "kpichart"}
                  onChange={_onChangeMode}
                  disabled={chart.type === "kpi" || chart.type === "avg"}
                  className="rounded"
                />
                <span className="text-sm">Show KPI on chart</span>
              </label>

              <label className="flex items-center gap-2">
                <input
                  type="checkbox"
                  checked={chart.showGrowth}
                  onChange={_onChangeGrowth}
                  disabled={chart.mode === "chart" && chart.type !== "kpi"}
                  className="rounded"
                />
                <span className="text-sm">Show growth</span>
              </label>

              <label className="flex items-center gap-2">
                <input
                  type="checkbox"
                  checked={chart.invertGrowth}
                  onChange={_onChangeInvertGrowth}
                  className="rounded"
                />
                <span className="text-sm"> Invert growth</span>
              </label>
            </div>
            <div className="h-8" />
          </div>
        )}

      {chart &&
        chart.type &&
        chart.ChartDatasetConfigs &&
        chart.ChartDatasetConfigs.length > 0 &&
        chart.type === "gauge" && (
          <div className="flex flex-col gap-1 mt-4">
            <div className="text-sm font-bold">Define gauge ranges</div>
            <div className="flex flex-col gap-1">
              {/*   {ranges.map((range, index) => (
                <div
                  key={index}
                  className={`flex flex-row ${
                    index === 0 ? "items-end" : "items-center"
                  } gap-2 w-full sm:flex-wrap md:flex-nowrap`}
                >
                  <Input
                    label={index === 0 ? "Min" : ""}
                    value={String(range.min)}
                    onChange={(e) =>
                      _onChangeRange(parseFloat(e.target.value), index, "min")
                    }
                    variant="bordered"
                    type="number"
                    step="0.01"
                    className="max-w-[150px]"
                  />
                  <Input
                    label={index === 0 ? "Max" : ""}
                    labelPlacement="outside"
                    value={String(range.max)}
                    onChange={(e) =>
                      _onChangeRange(parseFloat(e.target.value), index, "max")
                    }
                    variant="bordered"
                    type="number"
                    step="0.01"
                    className="max-w-[150px]"
                  />
                  <Input
                    label={index === 0 ? "Label" : ""}
                    labelPlacement="outside"
                    value={range.label}
                    onChange={(e) =>
                      _onChangeRange(e.target.value, index, "label")
                    }
                    variant="bordered"
                    size="sm"
                    className="max-w-[200px]"
                  />
                  <Popover>
                    <PopoverTrigger>
                      <div
                        style={{ backgroundColor: range.color }}
                        className="min-w-[30px] h-[30px] rounded-lg cursor-pointer border-2 border-divider"
                        aria-label="Select color"
                      />
                    </PopoverTrigger>
                    <PopoverContent>
                      <TwitterPicker
                        triangle={"hide"}
                        color={range.color}
                        onChange={(color) => _onChangeColor(color, index)}
                        colors={Object.values(chartColors).map((c) => c.hex)}
                      />
                    </PopoverContent>
                  </Popover>
                  {ranges.length > 1 && (
                    <Button
                      onClick={() => _onRemoveRange(index)}
                      variant="outline"
                      size="sm"
                    >
                      <CircleXIcon />
                    </Button>
                  )}
                </div>
              ))} */}
              <div className="h-4" />
              <div className="flex flex-row gap-2 items-center">
                <Button onClick={_onAddRange} variant="secondary" size="sm">
                  Add range
                  <PlusIcon className="ml-2" />
                </Button>
                {!isEqual(ranges, chart.ranges) && (
                  <Button
                    onClick={_onSaveRanges}
                    variant="secondary"
                    size="sm"
                    color="primary"
                  >
                    Save ranges
                  </Button>
                )}
                {rangeErrors && (
                  <div className="text-red-500 text-xs">{rangeErrors}</div>
                )}
              </div>
            </div>
          </div>
        )}
    </div>
  );
}

const styles = {
  topBuffer: {
    marginTop: 20,
  },
  modeSwitcher: {
    marginRight: 10,
  },
  chartCardContainer: {
    background: "white",
    border: "1px solid rgba(34,36,38,.15)",
    boxShadow: "0 1px 2px 0 rgb(34 36 38 / 15%)",
    borderRadius: 4,
    borderTopRightRadius: 0,
    borderBottomRightRadius: 0,
  },
  chartCardContainerSeparator: {
    background: "white",
    padding: 5,
    border: "1px solid rgba(34,36,38,.15)",
    marginRight: 10,
    marginLeft: -1,
    boxShadow: "0 1px 2px 0 rgb(34 36 38 / 15%)",
    borderRadius: 4,
    borderTopLeftRadius: 0,
    borderBottomLeftRadius: 0,
  },
  segmentContainer: {
    border: "none",
    boxShadow: "none",
  },
};
