...
export const AddSubDashboard = ({ open, toggleOpen }: AddSubDashboardProps) => {
  const t = useI18n();
  ...
  const createSubDashboard = useAction(createSubDashboardAction, {
    onSuccess: ({ data }) => {
      ...
      const newPath = `/missions/${missionIdMock}/sub-dashboard/${data?.data?.id}`;
